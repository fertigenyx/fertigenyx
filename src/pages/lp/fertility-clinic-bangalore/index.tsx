...
import { Fertility_Clinic_RelatedSearches } from '@/db/relatedSearchesDB';
import { SEOData } from '@/db/SEOData';
import FertilityClinicFAQs from '@/db/FertilityClinicFAQs';
...
const index = ({ doctors }) => {
  return (
    <div>
      <Head>
        <title>{SEOData.FertilityClinicBangalore.title}</title>
        <meta name='description' content={SEOData.FertilityClinicBangalore.description} />
        <meta name='keywords' content={SEOData.FertilityClinicBangalore.keywords} />
      </Head>
      <LPHeader />
      <main className='grid items-center justify-items-center gap-6 bg-gray-100 lg:gap-10 lg:p-8 lg:pb-20'>
        ...
        <FertilityClinicIndication />
        <Faq data={FertilityClinicFAQs} />
        <Cta />
        <RelatedSearches data={Fertility_Clinic_RelatedSearches} />
      </main>
    </div>
  );
};
